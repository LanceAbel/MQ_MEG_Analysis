function [traj, infStates] = tapas_hgf_whatworld_mod(r, p, varargin)
...
% All files generated were run using this
% Modification to HGF Model - see  See Scale_Prediction_Error.xlsx  ('Model 2' tab at bottom) for explanation 
MULTIPLIER_MODEL = [0	1.00	1.63	   2.13	   2.56	   2.96	    3.34];
CONSTANT_MODEL = max(MULTIPLIER_MODEL);
MAX_OF_TABLE = 2.5;

EXPONENT = 0.5;
CAP_MODEL = 0.5;
END_MULTIPLIER_MODEL = 0.886; % Necessary end-step multiplication to on average not add or subtract from the PE2 based on how much the tone frequency has changed
DIVISOR_FOR_PE2 = 1.4 ; % The above model tends to increase PE2, so decrese it a little at the end to keep the max PE2 close to 1.00
...
if ~isfield(r,'c_prc')
    error('tapas:hgf:ConfigRequired', 'Configuration required: before calling tapas_hgf_whatworld_mod, tapas_hgf_whatworld_config_mod has to be called.');
end
...
%     disp(r.u_orig(k-1))
    ...
    if not(ismember(k-1, r.ign))
        ...
        if r.u_orig(k) ~= r.u_orig(k-1) && k > 3  % Custom calculation for deviant

            % Lance: Modified prediction error; depends on the frequency change
            ...
            gap = r.u_orig(k) - r.u_orig(k-1);
            intermediate = log(max(CAP_MODEL,abs(gap))/CONSTANT_MODEL); 
            if isinf(intermediate)
                intermediate = -MAX_OF_TABLE;
            end
            scaled_change_in_tone = max(0,intermediate + MAX_OF_TABLE);
            scaled_change_in_tone_power = scaled_change_in_tone.^EXPONENT;
            scaled_change_in_tone_power = scaled_change_in_tone_power * END_MULTIPLIER_MODEL / DIVISOR_FOR_PE2;

            da1(k,:,u(k-1)) = (mu1(k,:,u(k-1)) -mu1hat(k,:,u(k-1)))*scaled_change_in_tone_power;
        else % Normal calculation for non-deviant
            da1(k,:,u(k-1)) = (mu1(k,:,u(k-1)) -mu1hat(k,:,u(k-1))); 
        end
